---
name: conectese-dev
description: "conectese development checklist — verifies templates sync, package integrity, and distribution correctness."
---

# conectese Development Checklist

You are running the conectese-dev verification skill inside the conectese repository.
Your job is to detect and report distribution issues before they reach users.

## How conectese Distribution Works

Understand this before checking anything:

- **`templates/`** → Copied by `src/init.js:copyCommonTemplates()` during `npx conectese init`
  and by `src/update.js` during `npx conectese update`. Everything in `templates/` except
...
- **`_conectese/core/*`** → Framework core files. MUST have a mirror copy in
  `templates/_conectese/core/*`. Any change to a core file that isn't synced to templates
  means `npx conectese init` delivers STALE content to new users.

- **`skills/*`** → Bundled skills catalog. Two distribution sub-types:
  - **Multi-file skills** (have scripts/assets/agents dirs alongside SKILL.md, e.g. `conectese-skill-creator`):
    also have a mirror in `templates/skills/*` so the full directory structure is copied during init.
    Check B verifies this sync.
  - **Single-file skills** (only `SKILL.md`, no subdirs, e.g. `conectese-agent-creator`):
...
conectese supports multiple IDEs. When a user runs `npx conectese init`, they choose which IDE(s) to install for. Files are copied from two places:
...
|------|-----------|----------|
| SHARED | Applies to all IDEs equally | `_conectese/core/`, `templates/` (excluding `ide-templates/`) |
...
| Antigravity | `templates/ide-templates/antigravity/` | `.agent/rules/conectese.md`, `.agent/workflows/conectese.md` |
| OpenCode | `templates/ide-templates/opencode/` | `AGENTS.md`, `.opencode/commands/conectese.md` |
...
- ❌ Wrong: Edit `_conectese/core/runner.pipeline.md` adding `if antigravity: run sequentially`
- ✅ Correct: Edit `templates/ide-templates/antigravity/.agent/rules/conectese.md` with the sequential execution instructions

**Legitimate shared file edit:**
- User asks: "Add a new researcher agent type to the architect."
- ✅ Fine: Edit `_conectese/core/architect.agent.yaml` because the change benefits ALL IDEs equally.
...
#### Check A: Core file sync (`_conectese/core/**` changed)

For EACH changed file matching `_conectese/core/**`:

1. Compute the expected template path: `templates/{same relative path}`
   Example: `_conectese/core/runner.pipeline.md` → `templates/_conectese/core/runner.pipeline.md`

2. Run diff:
   ```bash
   diff "_conectese/core/{file}" "templates/_conectese/core/{file}"
   ```

3. If diff shows differences:
   - **FAIL**: Report the file and show the diff summary
   - **Fix**: `cp _conectese/core/{file} templates/_conectese/core/{file}`

4. If template file doesn't exist:
   - **FAIL**: "Template missing for `_conectese/core/{file}`"
   - **Fix**: `mkdir -p templates/_conectese/core/{dir} && cp _conectese/core/{file} templates/_conectese/core/{file}`
...
5. Verify `installNonMcpSkills` filters out: `conectese-skill-creator`, type `mcp`, type `hybrid`, skills with `env`
...
#### Check J: IDE contamination in shared files (any `_conectese/core/**` or `templates/**` changed, excluding `templates/ide-templates/**`)

Scan shared files for IDE-specific conditional logic that should live in `templates/ide-templates/{ide}/` instead.

**Files to scan:**
- All files in `_conectese/core/`
...
**Exclusions:** Mentions inside this conectese-dev SKILL.md itself (documentation), and any comment explicitly labeled as a cross-reference.
