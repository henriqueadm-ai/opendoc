---
name: opendoc-dev
description: "opendoc development checklist — verifies templates sync, package integrity, and distribution correctness."
---

# opendoc Development Checklist

You are running the opendoc-dev verification skill inside the opendoc repository.
Your job is to detect and report distribution issues before they reach users.

## How opendoc Distribution Works

Understand this before checking anything:

- **`templates/`** → Copied by `src/init.js:copyCommonTemplates()` during `npx opendoc init`
  and by `src/update.js` during `npx opendoc update`. Everything in `templates/` except
...
- **`_opendoc/core/*`** → Framework core files. MUST have a mirror copy in
  `templates/_opendoc/core/*`. Any change to a core file that isn't synced to templates
  means `npx opendoc init` delivers STALE content to new users.

- **`skills/*`** → Bundled skills catalog. Two distribution sub-types:
  - **Multi-file skills** (have scripts/assets/agents dirs alongside SKILL.md, e.g. `opendoc-skill-creator`):
    also have a mirror in `templates/skills/*` so the full directory structure is copied during init.
    Check B verifies this sync.
  - **Single-file skills** (only `SKILL.md`, no subdirs, e.g. `opendoc-agent-creator`):
...
- **`src/update.js:PROTECTED_PATHS`** → Directories NEVER overwritten during update:
  `_opendoc/_memory`, `_opendoc/_investigations`, `agents`, `squads`.

## Multi-IDE Architecture

opendoc supports multiple IDEs. When a user runs `npx opendoc init`, they choose which IDE(s) to install for. Files are copied from two places:
...
|------|-----------|----------|
| SHARED | Applies to all IDEs equally | `_opendoc/core/`, `templates/` (excluding `ide-templates/`) |
...
| Antigravity | `templates/ide-templates/antigravity/` | `.agent/rules/opendoc.md`, `.agent/workflows/opendoc.md` |
| OpenCode | `templates/ide-templates/opencode/` | `AGENTS.md`, `.opencode/commands/opendoc.md` |
...
- ❌ Wrong: Edit `_opendoc/core/runner.pipeline.md` adding `if antigravity: run sequentially`
- ✅ Correct: Edit `templates/ide-templates/antigravity/.agent/rules/opendoc.md` with the sequential execution instructions

**Legitimate shared file edit:**
- User asks: "Add a new researcher agent type to the architect."
- ✅ Fine: Edit `_opendoc/core/architect.agent.yaml` because the change benefits ALL IDEs equally.
...
#### Check A: Core file sync (`_opendoc/core/**` changed)

For EACH changed file matching `_opendoc/core/**`:

1. Compute the expected template path: `templates/{same relative path}`
   Example: `_opendoc/core/runner.pipeline.md` → `templates/_opendoc/core/runner.pipeline.md`

2. Run diff:
   ```bash
   diff "_opendoc/core/{file}" "templates/_opendoc/core/{file}"
   ```

3. If diff shows differences:
   - **FAIL**: Report the file and show the diff summary
   - **Fix**: `cp _opendoc/core/{file} templates/_opendoc/core/{file}`

4. If template file doesn't exist:
   - **FAIL**: "Template missing for `_opendoc/core/{file}`"
   - **Fix**: `mkdir -p templates/_opendoc/core/{dir} && cp _opendoc/core/{file} templates/_opendoc/core/{file}`
...
3. Verify it includes all user-owned directories:
   - `_opendoc/_memory` (user preferences and company context)
   - `_opendoc/_investigations` (Sherlock investigation data)
...
5. Verify `installNonMcpSkills` filters out: `opendoc-skill-creator`, type `mcp`, type `hybrid`, skills with `env`
...
#### Check J: IDE contamination in shared files (any `_opendoc/core/**` or `templates/**` changed, excluding `templates/ide-templates/**`)

Scan shared files for IDE-specific conditional logic that should live in `templates/ide-templates/{ide}/` instead.

**Files to scan:**
- All files in `_opendoc/core/`
...
**Exclusions:** Mentions inside this opendoc-dev SKILL.md itself (documentation), and any comment explicitly labeled as a cross-reference.
