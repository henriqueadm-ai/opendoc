# Contributing to Conectese

Você está contribuindo para algo maior que código.

Conectese existe para liberar empresas do trabalho repetitivo. Cada skill que você cria, cada melhoria que você faz, permite que empreendedores e times dediquem seu tempo àquilo que máquinas não conseguem fazer: criatividade, relacionamento, decisão humana.
...
Conectese é um framework de orquestração multi-agente. Ajudamos empreendedores e profissionais não-técnicos a criar times de agentes de IA que trabalham juntos automaticamente.
...
- Reescrever `_conectese/core/` - prefira estender via skills e agents
- Aumentar significativamente a árvore de dependências - cada dependência nova é uma barreira a mais
- Funcionalidades de desenvolvimento de software - o conectese é para automatizar processos de negócio, não para criar apps
...
2. Rode `npx conectese init` em uma pasta de teste para experimentar o fluxo
...
Skills são o principal ponto de extensão do conectese. É a melhor forma de contribuir. Use a skill `conectese-skill-creator` para criar e iterar:

```
/conectese install conectese-skill-creator
```
...
O formato completo do `SKILL.md` está documentado em [`skills/conectese-skill-creator/references/skill-format.md`](skills/conectese-skill-creator/references/skill-format.md).
...
O conectese já vem com guias de copywriting, design de carrosséis, SEO e mais. Mas tem muito mais a ser coberto. Use a skill `conectese-agent-creator`:

```
/conectese install conectese-agent-creator
```
...
# Contributing to conectese (English)
...
- Rewriting `_conectese/core/` - prefer extending via skills and agents
- Significantly increasing the dependency tree - every new dependency is one more barrier
- Software development features - conectese is for automating business processes, not for building apps
...
2. Run `npx conectese init` in a test folder to experience the flow
...
Skills are conectese's primary extension point. This is the best way to contribute. Use the `conectese-skill-creator` skill to create and iterate:
...
The full `SKILL.md` format is documented in [`skills/conectese-skill-creator/references/skill-format.md`](skills/conectese-skill-creator/references/skill-format.md).
...
conectese ships with guides for copywriting, carousel design, SEO, and more. But there's much more to cover. Use the `conectese-agent-creator` skill:
