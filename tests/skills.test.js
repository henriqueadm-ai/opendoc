...
test('listInstalled excludes the built-in conectese-skill-creator skill', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'conectese-test-'));
  try {
    const skillsDir = join(dir, 'skills');
    await mkdir(join(skillsDir, 'conectese-skill-creator'), { recursive: true });
...
test('installSkill copies full directory including subdirs for conectese-skill-creator', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'conectese-test-'));
  try {
    await installSkill('conectese-skill-creator', dir);
    const skill = await readFile(join(dir, 'skills', 'conectese-skill-creator', 'SKILL.md'), 'utf-8');
    assert.ok(skill.length > 0);
    const scripts = await readdir(join(dir, 'skills', 'conectese-skill-creator', 'scripts'));
...
test('installSkill skips copy when src and dest resolve to the same path', async () => {
  // Simulates running init from inside the conectese repo itself
