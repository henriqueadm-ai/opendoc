...
1. You have already loaded:
   - The squad's `squad.yaml` (passed to you by the Opendoc skill)
   - The squad's `squad-party.csv` (all agent personas)
   - Company context from `_opendoc/_memory/company.md`
...
4. **Inject format context**: Check if the current step's frontmatter contains a `format:` field.
   If present:
   a. Read `_opendoc/core/best-practices/{format}.md` (e.g., `_opendoc/core/best-practices/instagram-feed.md`)
      - If the file does not exist → **WARNING**: "Format '{format}' not found in _opendoc/core/best-practices/. Skipping format injection." Continue without format.
...
2. **Update squad memory** — write to BOTH files (runs after Post-Completion Cleanup above):
   ...
   **Technical routing:** For any technical learning (bugs, workarounds, API behavior):
   - If it affects any squad (Playwright bugs, OS rendering quirks, API limits) → write to the appropriate `_opendoc/core/best-practices/` file instead of `memories.md`
...
- If a step file is missing, inform the user and suggest running `/opendoc edit {squad}` to fix.
