...
Browser sessions are stored as JSON files in `_opendoc/_browser_profile/`:

- Instagram: `_opendoc/_browser_profile/instagram.json`
- YouTube: `_opendoc/_browser_profile/youtube.json`
- Twitter/X: `_opendoc/_browser_profile/twitter.json`
- LinkedIn: `_opendoc/_browser_profile/linkedin.json`

**Loading a session (Playwright CLI):**
```bash
npx playwright open --load-storage=_opendoc/_browser_profile/{platform}.json {url}
```

**Saving a session (Playwright CLI):**
```bash
npx playwright open --save-storage=_opendoc/_browser_profile/{platform}.json {url}
```
...
Session files are stored in `_opendoc/_browser_profile/` as JSON files (one per platform). This directory is gitignored and persists between runs.
...
Users can clear all saved browser sessions at any time by deleting the `_opendoc/_browser_profile/` directory or individual JSON files. After deletion, the next investigation will require a fresh manual login.
