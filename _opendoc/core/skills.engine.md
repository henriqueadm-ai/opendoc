...
- **Skill catalog**: `https://github.com/renatoasse/opendoc/tree/main/skills`
- **Skill format reference**: `skills/opendoc-skill-creator/references/skill-format.md`
...
For the full SKILL.md specification, see `skills/opendoc-skill-creator/references/skill-format.md`.
...
2. **Fetch SKILL.md from GitHub**:
   ```
   https://raw.githubusercontent.com/renatoasse/opendoc/main/skills/<name>/SKILL.md
   ```
   - If fetch fails (404 or network error) → **ERROR**: "Skill '<name>' not found in the Opendoc catalog."
...
5. **Fetch additional files** (if the skill requires them):
   - If the SKILL.md frontmatter has `script.path` → fetch the script file from:
     `https://raw.githubusercontent.com/renatoasse/opendoc/main/skills/<name>/{script.path}`
...
1. Check if the `opendoc-skill-creator` skill is installed:
   - Look for `skills/opendoc-skill-creator/SKILL.md`

2. **If installed** → read `skills/opendoc-skill-creator/SKILL.md` and follow its
...
3. **If not installed** → inform the user:
   ...
   `/opendoc skills` → Install → `opendoc-skill-creator`"
...
3. **For each non-native skill**:
   ...
   c. **Verify MCP configuration** (if `type: mcp` or `type: hybrid`):
      ...
      - If missing → **ERROR**: "Skill '{skill}' is installed but its MCP server is not configured. Run `/opendoc skills` → Install to reconfigure."
...
2. **Fetch the catalog index**:
   ...
   ```
   https://raw.githubusercontent.com/renatoasse/opendoc/main/skills/README.md
   ```
