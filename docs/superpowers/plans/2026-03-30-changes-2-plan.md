...
**Architecture:** All changes target existing prompt/config files in `_conectese/core/` and `skills/`. Task 4 deletes 5 server files and rewrites the Template Designer SKILL.md.
...
| File | Action | Task |
|------|--------|------|
| `_conectese/core/prompts/discovery.prompt.md` | Modify | 1, 3 |
| `_conectese/core/prompts/design.prompt.md` | Modify | 2, 3, 5 |
| `_conectese/core/prompts/build.prompt.md` | Modify | 5 |
| `_conectese/core/runner.pipeline.md` | Modify | 5 |
...
> "Do you have specific profiles or channels you'd like me to investigate? I can analyze their real content to extract patterns, hooks, structures, and engagement data to make your team much smarter.
>
...
In `_conectese/core/prompts/discovery.prompt.md`, in the `## Communication Style` section, after the line:
```
...
In `_conectese/core/prompts/design.prompt.md`, find the line (around line 9):
```
...
```bash
git add _conectese/core/prompts/discovery.prompt.md _conectese/core/prompts/design.prompt.md
...
```bash
cd "d:/Coding Projects/conectese"
...
>
> A: `teams/{code}/_build/template-a.png`
> B: `teams/{code}/_build/template-b.png`
> C: `teams/{code}/_build/template-c.png`
>
...
In `_conectese/core/runner.pipeline.md`, find the checkpoint section (around line 307):
```markdown
#### If `type: checkpoint`
...
In `_conectese/core/prompts/design.prompt.md`, find the Phase G section (around the "Does this look good?" line). After:
```
...
In `_conectese/core/prompts/build.prompt.md`, find `## Step D: Present Summary`. After the summary template block (after the line `To modify it: /conectese edit {code}`), add:
...
```bash
git add _conectese/core/runner.pipeline.md _conectese/core/prompts/design.prompt.md _conectese/core/prompts/build.prompt.md
