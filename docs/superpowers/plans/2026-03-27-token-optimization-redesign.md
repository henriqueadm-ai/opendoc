...
**Files:**
- Create: `_opendoc/core/prompts/sherlock-shared.md`
- Create: `_opendoc/core/prompts/sherlock-instagram.md`
- Create: `_opendoc/core/prompts/sherlock-youtube.md`
- Create: `_opendoc/core/prompts/sherlock-twitter.md`
- Create: `_opendoc/core/prompts/sherlock-linkedin.md`
- Remove (later, Task 7): `_opendoc/core/prompts/sherlock.prompt.md`
...
Sherlock is dispatched by the Opendoc orchestrator to investigate reference profiles during squad creation. Each invocation investigates ONE profile on ONE platform.
...
Sherlock uses Playwright CLI with persistent session storage. Sessions are saved as JSON files in `_opendoc/_browser_profile/`.
...
ls _opendoc/_browser_profile/{platform}.json
```

- **If exists:** Use `--load-storage=_opendoc/_browser_profile/{platform}.json` for all navigation
...
2. Open interactive browser:
   ```bash
   npx playwright open --save-storage=_opendoc/_browser_profile/{platform}.json https://{platform-url}
   ```
...
Write this file to `_opendoc/core/prompts/sherlock-shared.md`.
...
1. Navigate to profile:
   ```bash
   npx playwright open --load-storage=_opendoc/_browser_profile/instagram.json "https://www.instagram.com/{username}/"
   ```
...
Write to `_opendoc/core/prompts/sherlock-instagram.md`.
...
Write to `_opendoc/core/prompts/sherlock-youtube.md`.
...
Write to `_opendoc/core/prompts/sherlock-twitter.md`.
...
Write to `_opendoc/core/prompts/sherlock-linkedin.md`.

- [ ] **Step 6: Verify all 5 Sherlock files exist and have content**

Run:
```bash
wc -l _opendoc/core/prompts/sherlock-shared.md _opendoc/core/prompts/sherlock-instagram.md _opendoc/core/prompts/sherlock-youtube.md _opendoc/core/prompts/sherlock-twitter.md _opendoc/core/prompts/sherlock-linkedin.md
```
...
```bash
git add _opendoc/core/prompts/sherlock-shared.md _opendoc/core/prompts/sherlock-instagram.md _opendoc/core/prompts/sherlock-youtube.md _opendoc/core/prompts/sherlock-twitter.md _opendoc/core/prompts/sherlock-linkedin.md
...
**Files:**
- Create: `_opendoc/core/prompts/discovery.prompt.md`
...
You are the Opendoc Discovery agent. Your role is to understand what the user wants to build through intelligent conversation, then produce a structured discovery.yaml file.
...
- `_opendoc/_memory/preferences.md` — user preferences (language, name)
...
Scan `_opendoc/core/best-practices/` directory for format files. Parse YAML frontmatter for `name` and `description`. Present as a numbered list grouped by platform. User can pick multiple (e.g., "1 3").
...
Write to `_opendoc/core/prompts/discovery.prompt.md`.

- [ ] **Step 2: Verify the file**

```bash
wc -l _opendoc/core/prompts/discovery.prompt.md
```
...
**Files:**
- Create: `_opendoc/core/prompts/design.prompt.md`
...
You are the Opendoc Design agent. Your role is to compose the squad structure based on Discovery and Investigation results.
...
- `_opendoc/core/best-practices/_catalog.yaml` — Available best-practices catalog
...
Write to `_opendoc/core/prompts/design.prompt.md`.
...
```bash
wc -l _opendoc/core/prompts/design.prompt.md
```
...
**Files:**
- Create: `_opendoc/core/prompts/build.prompt.md`
...
You are the Opendoc Build agent. Your role is to take an approved design.yaml and generate all squad files.
...
- Read `_opendoc/core/best-practices/{best-practice}.md` — On demand, only the ones needed
...
✓ Referenced best-practices exist in _opendoc/core/best-practices/
```
...
Write to `_opendoc/core/prompts/build.prompt.md`.
...
```bash
wc -l _opendoc/core/prompts/build.prompt.md
```
...
Read `.claude/skills/opendoc/SKILL.md` to understand the current structure (169 lines).
...
When the user runs `/opendoc create`:
...
2. Dispatch Discovery subagent:
   - Read `_opendoc/core/prompts/discovery.prompt.md`
   - Read `_opendoc/_memory/company.md`
   - Read `_opendoc/_memory/preferences.md`
...
For each target in `investigation.targets`:
   ...
   2. Read `_opendoc/core/prompts/sherlock-shared.md`
   3. Read `_opendoc/core/prompts/sherlock-{platform}.md`
...
2. Dispatch Design subagent:
   - Read `_opendoc/core/prompts/design.prompt.md`
...
1. Dispatch Build subagent:
   - Read `_opendoc/core/prompts/build.prompt.md`
...
```
| `/opendoc create <description>` | Run Create Squad — Phased Orchestration |
```
...
```bash
wc -l .claude/skills/opendoc/SKILL.md
```
...
```bash
git add .claude/skills/opendoc/SKILL.md
...
```yaml
    create-squad: |
      ...
      Each phase is a separate prompt in `_opendoc/core/prompts/`:
...
```bash
wc -l _opendoc/core/architect.agent.yaml
```
...
**Files:**
- Remove: `_opendoc/core/prompts/sherlock.prompt.md`

- [ ] **Step 1: Search for references to old sherlock path**

```bash
grep -r "sherlock.prompt.md" _opendoc/ .claude/ --include="*.md" --include="*.yaml" --include="*.yml"
```
...
```bash
git rm _opendoc/core/prompts/sherlock.prompt.md
```
...
ls -la _opendoc/core/prompts/discovery.prompt.md
ls -la _opendoc/core/prompts/design.prompt.md
ls -la _opendoc/core/prompts/build.prompt.md

echo "=== Platform-specific Sherlock ==="
ls -la _opendoc/core/prompts/sherlock-shared.md
ls -la _opendoc/core/prompts/sherlock-instagram.md
ls -la _opendoc/core/prompts/sherlock-youtube.md
ls -la _opendoc/core/prompts/sherlock-twitter.md
ls -la _opendoc/core/prompts/sherlock-linkedin.md

echo "=== Old sherlock removed ==="
ls -la _opendoc/core/prompts/sherlock.prompt.md 2>&1 || echo "REMOVED (expected)"
...
grep "discovery.prompt.md" .claude/skills/opendoc/SKILL.md
grep "design.prompt.md" .claude/skills/opendoc/SKILL.md
grep "build.prompt.md" .claude/skills/opendoc/SKILL.md
grep "sherlock-shared.md" .claude/skills/opendoc/SKILL.md

# Check no dangling references to old files
grep -r "sherlock.prompt.md" _opendoc/ .claude/ --include="*.md" --include="*.yaml"
```
...
wc -l _opendoc/core/prompts/sherlock-shared.md
wc -l _opendoc/core/prompts/sherlock-instagram.md
echo "Phase 3 (Design):"
wc -l _opendoc/core/prompts/design.prompt.md
echo "Phase 4 (Build):"
wc -l _opendoc/core/prompts/build.prompt.md
