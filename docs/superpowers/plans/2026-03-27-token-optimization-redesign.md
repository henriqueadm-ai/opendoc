...
**Files:**
- Create: `_conectese/core/prompts/sherlock-shared.md`
- Create: `_conectese/core/prompts/sherlock-instagram.md`
- Create: `_conectese/core/prompts/sherlock-youtube.md`
- Create: `_conectese/core/prompts/sherlock-twitter.md`
- Create: `_conectese/core/prompts/sherlock-linkedin.md`
- Remove (later, Task 7): `_conectese/core/prompts/sherlock.prompt.md`
...
Sherlock is dispatched by the Conectese orchestrator to investigate reference profiles during team creation. Each invocation investigates ONE profile on ONE platform.
...
Sherlock uses Playwright CLI with persistent session storage. Sessions are saved as JSON files in `_conectese/_browser_profile/`.
...
ls _conectese/_browser_profile/{platform}.json
```

- **If exists:** Use `--load-storage=_conectese/_browser_profile/{platform}.json` for all navigation
...
2. Open interactive browser:
   ```bash
   npx playwright open --save-storage=_conectese/_browser_profile/{platform}.json https://{platform-url}
   ```
...
Saved to `teams/{team-name}/_investigations/{username}/raw-content.md`.
...
Saved to `teams/{team-name}/_investigations/{username}/pattern-analysis.md`.
...
Saved to `teams/{team-name}/_investigations/consolidated-analysis.md`.
...
Write this file to `_conectese/core/prompts/sherlock-shared.md`.
...
Write to `_conectese/core/prompts/sherlock-instagram.md`.
...
Write to `_conectese/core/prompts/sherlock-youtube.md`.
...
Write to `_conectese/core/prompts/sherlock-twitter.md`.
...
Write to `_conectese/core/prompts/sherlock-linkedin.md`.

- [ ] **Step 6: Verify all 5 Sherlock files exist and have content**

Run:
```bash
wc -l _conectese/core/prompts/sherlock-shared.md _conectese/core/prompts/sherlock-instagram.md _conectese/core/prompts/sherlock-youtube.md _conectese/core/prompts/sherlock-twitter.md _conectese/core/prompts/sherlock-linkedin.md
```
...
```bash
git add _conectese/core/prompts/sherlock-shared.md _conectese/core/prompts/sherlock-instagram.md _conectese/core/prompts/sherlock-youtube.md _conectese/core/prompts/sherlock-twitter.md _conectese/core/prompts/sherlock-linkedin.md
...
**Files:**
- Create: `_conectese/core/prompts/discovery.prompt.md`
...
You are the Conectese Discovery agent. Your role is to understand what the user wants to build through intelligent conversation, then produce a structured discovery.yaml file.

## Persona

You are a strategic systems thinker who sees organizations as interconnected workflows. Patient with all users, always explains in plain language. Believes the best team is the simplest one that gets the job done.
...
- `_conectese/_memory/company.md` — company context
- `_conectese/_memory/preferences.md` — user preferences (language, name)
...
**Research domain:**
- "What sources should the team use?" (web search, specific sites, uploaded files, APIs)
...
**Analysis domain:**
- "What data inputs does the team need?" (files, APIs, databases, web)
...
"Does this team need any external tools or integrations?"
...
### 7. Target Formats (content teams only)

If domain is `content`, ask about target formats:

"Which content formats should this team produce?"

Scan `_conectese/core/best-practices/` directory for format files. Parse YAML frontmatter for `name` and `description`. Present as a numbered list grouped by platform. User can pick multiple (e.g., "1 3").

Save selected format IDs to discovery.yaml.

For NON-content teams: skip this question entirely.
...
After confirmation, write this file to `teams/{code}/_build/discovery.yaml` (where `{code}` is the team code derived from purpose):
...
- NEVER start designing the team — that's Phase 3
- Keep the conversation focused and efficient — maximum 8 questions total
```

Write to `_conectese/core/prompts/discovery.prompt.md`.

- [ ] **Step 2: Verify the file**

```bash
wc -l _conectese/core/prompts/discovery.prompt.md
```
...
**Context:** This replaces the architect's Phases 2-4 (Research, Extraction, Design). The Design phase reads discovery.yaml + investigation results, proposes team structure, and outputs design.yaml. This is where AI adds the most value.

**Files:**
- Create: `_conectese/core/prompts/design.prompt.md`
...
You are the Conectese Design agent. Your role is to compose the team structure based on Discovery and Investigation results.

## Context Loading

Read these files before starting:
- `teams/{code}/_build/discovery.yaml` — Discovery output
- `_conectese/_memory/company.md` — Company context
- `_conectese/_memory/preferences.md` — User preferences
- `_conectese/core/best-practices/_catalog.yaml` — Available best-practices catalog

If investigation was performed:
- `teams/{code}/_investigations/*/raw-content.md` — Raw content from each profile
- `teams/{code}/_investigations/*/pattern-analysis.md` — Pattern analyses
...
Based on the team's purpose and domain from discovery.yaml:

1. Review each catalog entry's `whenToUse` field
2. Select entries matching the team's needs
3. Read the FULL content of each selected best-practice file
4. Use this knowledge to design better agents

Do NOT read all 22 files — only those relevant to this team.
...
Include: research/creative execution modes, checkpoints, content team patterns,
...
After user approves, write to `teams/{code}/_build/design.yaml`:
...
```bash
wc -l _conectese/core/prompts/design.prompt.md
```
...
and team design presentation. Loads best-practices selectively.
Outputs design.yaml with full team structure for Build phase."
```
...
**Context:** This replaces the architect's Phases 5-6 (Build + Validate). The Build phase reads design.yaml and generates all team files. It's mostly mechanical — templates and validation.

**Files:**
- Create: `_conectese/core/prompts/build.prompt.md`
...
You are the Conectese Build agent. Your role is to take an approved design.yaml and generate all team files.

## Context Loading

Read these files:
- `teams/{code}/_build/design.yaml` — Approved design from Phase 3
- `teams/{code}/_build/discovery.yaml` — Original discovery data
- `_conectese/_memory/company.md` — Company context
- `_conectese/_memory/preferences.md` — User preferences

For each best-practice referenced in design.yaml agents:
- Read `_conectese/core/best-practices/{best-practice}.md` — On demand, only the ones needed

If investigation was performed:
- Read `teams/{code}/_investigations/*/raw-content.md` as needed for output examples
...
Include: team.yaml format, team-party.csv format, agent .agent.md format
...
```
✓ teams/{code}/team.yaml exists and is valid YAML
...
✓ Referenced best-practices exist in _conectese/core/best-practices/
```
...
Write to `_conectese/core/prompts/build.prompt.md`.
...
```bash
wc -l _conectese/core/prompts/build.prompt.md
```
...
Generates all team files from approved design.yaml.
...
**Files:**
- Modify: `.claude/skills/conectese/SKILL.md`

- [ ] **Step 1: Read current SKILL.md**

Read `.claude/skills/conectese/SKILL.md` to understand the current structure (169 lines).
...
Replace the current `Loading Agents` section and add a new `Create Team — Phased Orchestration` section:
...
1. Check if `teams/{name}/_build/discovery.yaml` already exists (resume support)
...
2. Dispatch Discovery subagent:
   ...
   - Read `_conectese/_memory/company.md`
   - Read `_conectese/_memory/preferences.md`
...
For each target in `investigation.targets`:
   ...
   2. Read `_conectese/core/prompts/sherlock-shared.md`
   3. Read `_conectese/core/prompts/sherlock-{platform}.md`
   4. Dispatch Sherlock subagent (one per URL, can run in parallel):
      - Pass: URL, investigation_mode, output directory, team name
      - Use fast model tier
...
1. Check if `teams/{code}/_build/design.yaml` already exists (resume support)
   - If exists: read it, show summary, ask if user wants to redo or continue
...
```
| `/conectese create <description>` | Run Create Team — Phased Orchestration |
```

Keep the existing `Loading Agents` section but update it to clarify it's for team agents during RUN, not for the Architect during CREATE.

- [ ] **Step 3: Verify the updated SKILL.md**

```bash
wc -l .claude/skills/conectese/SKILL.md
```
...
```bash
git add .claude/skills/conectese/SKILL.md
...
**Context:** The architect's create-team workflow (1,000+ lines) is now handled by the phase prompts. The architect.agent.yaml should be reduced to only contain the shared persona, the edit/list/delete workflows, and a reference to the phased create flow.
...
- Lines 1096-1131: edit-team, list-teams, delete-team workflows

Remove:
- Lines 67-1095: The entire `create-team` workflow (now in discovery/design/build prompts)

Replace the create-team workflow with a brief reference:
...
```yaml
    create-team: |
      ...
      - `design.prompt.md` — Phase 3: Team architecture
...
```bash
wc -l _conectese/core/architect.agent.yaml
```
...
git commit -m "refactor(architect): remove create-team workflow
...
**Files:**
- Remove: `_conectese/core/prompts/sherlock.prompt.md`

- [ ] **Step 1: Search for references to old sherlock path**

```bash
grep -r "sherlock.prompt.md" _conectese/ .claude/ --include="*.md" --include="*.yaml" --include="*.yml"
```
...
```bash
git rm _conectese/core/prompts/sherlock.prompt.md
```
...
ls -la _conectese/core/prompts/discovery.prompt.md
ls -la _conectese/core/prompts/design.prompt.md
ls -la _conectese/core/prompts/build.prompt.md

echo "=== Platform-specific Sherlock ==="
ls -la _conectese/core/prompts/sherlock-shared.md
ls -la _conectese/core/prompts/sherlock-instagram.md
ls -la _conectese/core/prompts/sherlock-youtube.md
ls -la _conectese/core/prompts/sherlock-twitter.md
ls -la _conectese/core/prompts/sherlock-linkedin.md

echo "=== Old sherlock removed ==="
ls -la _conectese/core/prompts/sherlock.prompt.md 2>&1 || echo "REMOVED (expected)"
...
grep "discovery.prompt.md" .claude/skills/conectese/SKILL.md
grep "design.prompt.md" .claude/skills/conectese/SKILL.md
grep "build.prompt.md" .claude/skills/conectese/SKILL.md
grep "sherlock-shared.md" .claude/skills/conectese/SKILL.md

# Check no dangling references to old files
grep -r "sherlock.prompt.md" _conectese/ .claude/ --include="*.md" --include="*.yaml"
```
...
wc -l _conectese/core/prompts/sherlock-shared.md
wc -l _conectese/core/prompts/sherlock-instagram.md
echo "Phase 3 (Design):"
wc -l _conectese/core/prompts/design.prompt.md
echo "Phase 4 (Build):"
wc -l _conectese/core/prompts/build.prompt.md
