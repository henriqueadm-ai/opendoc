...
Add a **FULL SPRITE REVIEW** function to the `conectese-dashboard-design` skill as **Phase 7** — an optional phase invoked when the user explicitly requests a full visual audit of all sprites in the dashboard. It replaces the normal phases 2–6 for that session.
