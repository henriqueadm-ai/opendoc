...
This rule applies to all Conectese interactions, not just the Architect.
...
**Files:** `_conectese/core/prompts/discovery.prompt.md`, `_conectese/core/prompts/design.prompt.md`, `_conectese/core/prompts/build.prompt.md`, `_conectese/core/architect.agent.yaml`
...
2. **Step 2 — Save session:** After user confirms, run `npx playwright open --save-storage=_conectese/_browser_profile/instagram.json https://instagram.com` to capture the already-authenticated session. This command completes quickly since the user is already logged in.

**Files:** `_conectese/core/prompts/sherlock-shared.md`
...
> - Saved sessions are in `_conectese/_browser_profile/`
...
**Files:** `_conectese/core/runner.pipeline.md`
...
**Files:** `_conectese/core/best-practices/research.md`, `_conectese/core/prompts/design.prompt.md` (agent generation instructions)
