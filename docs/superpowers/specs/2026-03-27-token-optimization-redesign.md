# Conectese Token Optimization — CREATE Flow Redesign
...
The current `/conectese create` flow is a monolithic single-session process that:
...
5. Present summary to user:
   "Squad created:
    ...
    Ready to run with /conectese run {name}"
```
...
The `/conectese` skill entry point orchestrates phase transitions:
...
If a session is interrupted, the user can resume by running `/conectese create` again — the orchestrator checks which `_build/` artifacts already exist and resumes from the next incomplete phase.
