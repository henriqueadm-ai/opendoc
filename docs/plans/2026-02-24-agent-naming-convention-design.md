...
2. **First name:** Common human name in the user's Output Language (from `_conectese/_memory/preferences.md`)
3. **Last name:** Playful reference to the agent's specialty or profession
4. **Uniqueness:** Each agent in a team uses a different initial letter
...
The agent name language comes from `_conectese/_memory/preferences.md → Output Language`, which is set during `npx conectese init`.
...
#### 2. Template agents (instagram-content team)
...
### Not Changed
- `team.yaml` — references file paths, not names
