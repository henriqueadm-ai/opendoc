...
Conectese is expected to grow to 100+ skills. Bundling all skills inside the npm package forces every user to download content they don't need. Skills should be distributed on-demand, outside the installed package.

## Approach

GitHub Raw Content + Static Manifest (zero infrastructure, CDN-backed, no rate limits).

Skills live in a `skills/` folder in the main conectese GitHub repository (not in `templates/`). A `manifest.json` lists all available skills. Users browse and install only what they want, via CLI or `/conectese`.
...
│   └── ...
├── templates/                        ← UNCHANGED
│   └── .claude/skills/conectese/SKILL.md
├── src/
│   ├── skills.js                     ← NEW — fetch/install/remove logic
│   ├── init.js
│   └── update.js
└── bin/
    └── conectese.js                    ← updated with new subcommands
```
...
```js
const REGISTRY_BASE = 'https://raw.githubusercontent.com/<owner>/conectese/main';
...
New subcommands registered in `bin/conectese.js`, logic in `src/skills.js`:

| Command | Action |
|---|---|
| `conectese skills list` | Fetch manifest, show available vs installed |
| `conectese skills install <id>` | Download SKILL.md → `.claude/skills/<id>/SKILL.md` |
| `conectese skills remove <id>` | Delete `.claude/skills/<id>/` |
| `conectese skills update` | Re-download all installed skills |
...
- Community skill publishing (MVP: Conectese team only)
