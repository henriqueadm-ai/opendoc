...
**Goal:** Build an on-demand skills distribution system so users can browse and install Conectese skills from the central GitHub repository, both via CLI and from within `/conectese`.

**Architecture:** A `skills/` folder in the conectese repo hosts skill files and a `manifest.json` registry. A new `src/skills.js` module handles fetch/install/remove using Node.js native `fetch`. The CLI gains `skills list|install|remove|update` subcommands. The `SKILL.md` gains a `/conectese skills` command route and browser flow.
...
```markdown
# Conectese Skills Registry

This folder contains all available Conectese skills distributed via `npx conectese skills install`.
...
const MANIFEST_URL = 'https://raw.githubusercontent.com/conectese-ai/conectese/main/skills/manifest.json';
const SKILL_URL_BASE = 'https://raw.githubusercontent.com/conectese-ai/conectese/main/skills';
...
test('listInstalled excludes the built-in conectese skill', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'conectese-test-'));
  try {
    const skillsDir = join(dir, '.claude', 'skills');
    await mkdir(join(skillsDir, 'conectese'), { recursive: true });
...
export async function listInstalled(targetDir) {
  try {
    ...
    return entries
      .filter((e) => e.isDirectory() && e.name !== 'conectese')
      .map((e) => e.name);
  } catch {
    return [];
  }
...
```json
  ...
  "skillsInstallHint": "Install with: conectese skills install <id>",
...
```json
  ...
  "skillsInstallHint": "Instale com: conectese skills install <id>",
...
```json
  ...
  "skillsInstallHint": "Instala con: conectese skills install <id>",
  "skillsInstalling": "Instalando {id}...",
  "skillsInstalled": "✅ Instalada: {id}",
...
## Task 5: Wire up `bin/conectese.js` with skills subcommands
...
async function runInstall(id, targetDir) {
  if (!id) {
    console.log('\n  Usage: conectese skills install <id>\n');
    return;
  }
...
async function runRemove(id, targetDir) {
  if (!id) {
    console.log('\n  Usage: conectese skills remove <id>\n');
    return;
  }
...
**Step 2: Update `bin/conectese.js`**
...
if (command === 'init') {
  ...
  Usage:
    ...
    npx conectese update                  Update Conectese to the latest version
    npx conectese skills list             Browse available skills
    npx conectese skills install <id>     Install a skill
    npx conectese skills remove <id>      Remove a skill
    npx conectese skills update           Update all installed skills

  Learn more: https://github.com/your-org/conectese
  `);
...
```bash
git add src/skills-cli.js bin/conectese.js
...
- Modify: `templates/.claude/skills/conectese/SKILL.md` (keep in sync)

**Step 1: Add skills route to the Command Routing table**

In both SKILL.md files, find the Command Routing table and add this row after the `/conectese tools` row:

```markdown
| `/conectese skills` | Load Skills Browser → Browse & install skills |
```
...
```markdown
If the user selects "More options", present a second AskUserQuestion:
- **Skills** — Browse and install skills from the Conectese registry
- **Tools** — Manage tool integrations (MCP servers, scripts, APIs)
- **Company profile** — View or update your company information
...
```
SKILLS
  /conectese skills           Browse available skills
  /conectese skills install <id>  Install a skill
  /conectese skills remove <id>   Remove a skill
```
...
1. Use WebFetch to fetch `https://raw.githubusercontent.com/conectese-ai/conectese/main/skills/manifest.json`
   - If fetch fails, show: "Could not reach the skills registry. Check your connection."
2. Use Glob `.claude/skills/*/` to detect installed skills (exclude the `conectese` folder itself)
...
5. To install: WebFetch `https://raw.githubusercontent.com/conectese-ai/conectese/main/skills/<id>/SKILL.md`, then Write to `.claude/skills/<id>/SKILL.md`
...
```bash
git add .claude/skills/conectese/SKILL.md templates/.claude/skills/conectese/SKILL.md
git commit -m "feat: add skills browser to /conectese skill (menu + command route + browser flow)"
```
...
> **Note:** Before shipping, update the placeholder `conectese-ai/conectese` in `src/skills.js` and `SKILL.md` to the real GitHub owner/repo once it is public.

Files to update:
- `src/skills.js` — `REGISTRY_BASE` constant
- `.claude/skills/conectese/SKILL.md` — WebFetch URLs in "Loading the Skills Browser"
- `templates/.claude/skills/conectese/SKILL.md` — same

```bash
git add src/skills.js .claude/skills/conectese/SKILL.md templates/.claude/skills/conectese/SKILL.md
