...
**Goal:** Add a Sherlock content investigator that uses Playwright to extract and analyze real content from social media profiles during team creation, producing raw content dumps and pattern analyses that enrich the Architect's team design.

**Architecture:** Sherlock is a subagent the Architect dispatches during a new Phase 1.5 (Investigation Setup) of the create-team workflow. One Sherlock subagent runs per profile URL in parallel. Each uses Playwright MCP for navigation and yt-dlp + Whisper for video transcription. Output is saved to `_conectese/_investigations/{team-code}/` as raw-content.md + pattern-analysis.md per profile, plus a consolidated-analysis.md across all profiles.
...
**Files:**
- Create: `templates/_conectese/core/prompts/sherlock.prompt.md`
...
during team creation (Phase 1.5).
...
The Architect uses Sherlock during create-team Phase 1.5 when the user provides
reference profile URLs. Sherlock is NOT a team agent — it's a tool the Architect
uses to gather intelligence for better team design.
...
The profile is stored at `_conectese/_browser_profile/`.
...
Save to: `_conectese/_investigations/{team-code}/{platform}-{username}/raw-content.md`
...
Save to: `_conectese/_investigations/{team-code}/{platform}-{username}/pattern-analysis.md`
...
1. {Specific recommendation for how the team should structure content}
...
Save to: `_conectese/_investigations/{team-code}/consolidated-analysis.md`

Generated by the Architect after all Sherlock subagents complete.

Template:

# Consolidated Investigation: {team-code}
Date: {YYYY-MM-DD}
...
## Recommended Framework for Team
...
4. **domain-framework.md** — Use the Recommended Framework from consolidated analysis
   as the basis for the team's operational framework
...
The Architect recommends based on the team type being created:
- Carousel team → recommend analyzing carousels + reels
- Video team → recommend long-form videos
- Newsletter team → recommend articles + long posts
- Generic content → recommend diverse mix
```

**Step 2: Verify the file was created correctly**

Run: `wc -l templates/_conectese/core/prompts/sherlock.prompt.md`
Expected: 300+ lines

**Step 3: Commit**

```bash
git add templates/_conectese/core/prompts/sherlock.prompt.md
...
The `_conectese/_investigations/` directory needs to exist when a user initializes Conectese.

**Files:**
- Modify: `templates/_conectese/_investigations/.gitkeep` (create as template)
...
Expected: FAIL — `_conectese/_investigations` directory does not exist

**Step 3: Create the template directory marker**

Create file: `templates/_conectese/_investigations/.gitkeep`
Content: empty file
...
```bash
git add templates/_conectese/_investigations/.gitkeep tests/init.test.js
...
This is the core integration task. The Architect's create-team workflow needs a new phase between Discovery and Research.

**Files:**
- Modify: `templates/_conectese/core/architect.agent.yaml` (lines 62-131 are the create-team workflow)
...
```yaml
- When user provides reference profile URLs, dispatch Sherlock investigator subagents to extract real content patterns before designing the team
```

**Step 2: Add Phase 1.5 to create-team workflow**
...
```yaml
      ...
      and engagement data to make your team smarter.
      ...
      2. **Configure investigation per platform** using AskUserQuestion:
         ...
         - Content types to analyze (multiSelect, with smart defaults based on team type)
         - Number of contents to analyze (with recommendation)

         Smart defaults based on team type:
         - Carousel/image team → recommend: carousels + reels, quantity 10
         - Video team → recommend: long videos, quantity 5
         - Newsletter/article team → recommend: articles + posts, quantity 10
      ...
      4. **Read Sherlock instructions**:
         Read `_conectese/core/prompts/sherlock.prompt.md` for the full investigation guide.

      5. **Create investigation directory**:
         Create `_conectese/_investigations/{team-code}/` via mkdir.
...
```yaml
      ...
      Read `_conectese/_investigations/{team-code}/consolidated-analysis.md` and
      ...
      - **Output Examples**: Use the highest-engagement real content from
        raw-content.md as the basis for output examples. Adapt to the team's
      ...
      When investigation data is present, mark the team as "investigation-enriched"
      in team.yaml metadata:
      ```yaml
      investigation:
        enriched: true
        profiles_analyzed: 3
        date: 2026-02-24
        dir: _conectese/_investigations/{team-code}
      ```
```
...
```markdown
- `_conectese/_investigations/` — Sherlock content investigations (profile analyses)
```

After the `_conectese/_memory/` line.
...
Add investigation reference to the Conectese skill help.

**Files:**
- Modify: `templates/.claude/skills/conectese/SKILL.md`
...
```markdown
| `/conectese create <desc>` with URLs | Load Architect → Create Team with Sherlock investigation |
```

In the help text section, update the EXAMPLES:

```
  /conectese create "Instagram carousel team" (+ provide reference profile URLs when asked)
```

**Step 2: Commit**

```bash
git add templates/.claude/skills/conectese/SKILL.md
...
Read `templates/_conectese/core/prompts/sherlock.prompt.md` and verify:
...
Read `templates/_conectese/core/architect.agent.yaml` and verify:
...
- [ ] team.yaml metadata for investigation-enriched teams
...
- `_conectese/core/prompts/sherlock.prompt.md` — exists
- `_conectese/_investigations/` — created by init
...
|--------|------|---------------------|
| Create | `templates/_conectese/core/prompts/sherlock.prompt.md` | ~500 |
| Create | `templates/_conectese/_investigations/.gitkeep` | 0 (empty) |
| Modify | `templates/_conectese/core/architect.agent.yaml` | +80-100 |
| Modify | `templates/CLAUDE.md` | +3 |
| Modify | `templates/.claude/skills/conectese/SKILL.md` | +3 |
