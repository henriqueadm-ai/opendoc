...
**Architecture:** Sherlock is a subagent the Architect dispatches during a new Phase 1.5 (Investigation Setup) of the create-squad workflow. One Sherlock subagent runs per profile URL in parallel. Each uses Playwright MCP for navigation and yt-dlp + Whisper for video transcription. Output is saved to `_conectese/_investigations/{squad-code}/` as raw-content.md + pattern-analysis.md per profile, plus a consolidated-analysis.md across all profiles.
...
Save to: `_conectese/_investigations/{squad-code}/{platform}-{username}/raw-content.md`
...
Save to: `_conectese/_investigations/{squad-code}/{platform}-{username}/pattern-analysis.md`
...
Save to: `_conectese/_investigations/{squad-code}/consolidated-analysis.md`
...
**Files:**
- Modify: `templates/_conectese/core/architect.agent.yaml` (lines 62-131 are the create-squad workflow)
...
```yaml
      ...
      5. **Create investigation directory**:
         Create `_conectese/_investigations/{squad-code}/` via mkdir.
...
```yaml
      ...
      Read `_conectese/_investigations/{squad-code}/consolidated-analysis.md` and
      ...
      investigation:
        ...
        profiles_analyzed: 3
        date: 2026-02-24
        dir: _conectese/_investigations/{squad-code}
      ```
```
...
```markdown
| `/conectese create <desc>` with URLs | Load Architect → Create Squad with Sherlock investigation |
```

In the help text section, update the EXAMPLES:

```
  /conectese create "Instagram carousel squad" (+ provide reference profile URLs when asked)
```
