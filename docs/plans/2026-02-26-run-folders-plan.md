...
**Goal:** Each team execution creates a dedicated subfolder in `output/` so all run artifacts are grouped together.
...
3. **Create run folder**:
   - Path: `teams/{name}/output/YYYY-MM-DD-{slug}/`
   - Create subdirectories: `drafts/` and `final/`
   - If path already exists, append `-2`, `-3`, etc.
   - Example: `teams/instagram-content/output/2026-02-26-inteligencia-artificial/`

4. **Move step 1 output**: Move the staging file to the run folder
   - From: `teams/{name}/output/drafts/{filename}` → To: `{run-folder}/drafts/{filename}`
...
```
1. Save final output to `teams/{name}/output/YYYY-MM-DD/{filename}.md`
```
To:
```
...
### Task 3: Update step files for instagram-content team (steps 2-4)

**Files:**
- Modify: `teams/instagram-content/pipeline/steps/step-02-ideation.md:5,7` (frontmatter)
- Modify: `teams/instagram-content/pipeline/steps/step-03-writing.md:5,7` (frontmatter)
- Modify: `teams/instagram-content/pipeline/steps/step-04-review.md:5,6` (frontmatter)
- Modify: `teams/instagram-content/pipeline/steps/step-03-writing.md:113` (body text)
...
Change line 113:
```markdown
1. Read the review feedback from `{team-root}/output/drafts/review.md`
```
To:
```markdown
...
git add teams/instagram-content/pipeline/steps/step-03-writing.md
git add teams/instagram-content/pipeline/steps/step-04-review.md
...
**Files:**
- Modify: `templates/teams/instagram-content/pipeline/steps/step-02-ideation.md:5,7`
- Modify: `templates/teams/instagram-content/pipeline/steps/step-03-writing.md:5,7,113`
- Modify: `templates/teams/instagram-content/pipeline/steps/step-04-review.md:5,6`
...
```bash
git add templates/teams/instagram-content/pipeline/steps/step-02-ideation.md
git add templates/teams/instagram-content/pipeline/steps/step-03-writing.md
git add templates/teams/instagram-content/pipeline/steps/step-04-review.md
git commit -m "feat(steps): mirror run-folder changes to template steps"
```

---

### Task 5: Update team.yaml files

**Files:**
- Modify: `teams/instagram-content/team.yaml:31,39-40,48-49,55-56`
- Modify: `templates/teams/instagram-content/team.yaml:31,39-40,48-49,55-56`

**Step 1: Update teams/instagram-content/team.yaml**
...
**Step 2: Apply identical changes to template team.yaml**

Same changes to `templates/teams/instagram-content/team.yaml`.

**Step 3: Commit**

```bash
git add teams/instagram-content/team.yaml
git add templates/teams/instagram-content/team.yaml
git commit -m "feat(yaml): update team.yaml pipeline paths to use {run-folder}"
```
...
Change line 439:
```
      8. `teams/{code}/output/` — Empty output directory (create with mkdir)
```
To:
```
      8. `teams/{code}/output/` — Output directory (create with mkdir)
         - Also create `teams/{code}/output/drafts/` — Staging area for step 1
...
```
      - [ ] Step 1's outputFile uses `{team-root}/output/drafts/` (staging area)
...
- Step 1 `outputFile`: `{team-root}/output/drafts/research.md` (staging)
...
**Step 3: Verify team.yaml matches step files**

Read team.yaml and confirm paths match step file frontmatter for all steps.

**Step 4: Verify templates match team files**

Diff template files against team files to confirm they're in sync.
