...
**Tech Stack:** YAML tool definitions, Conectese tools engine
...
**Files:**
- Create: `_conectese/tools/registry/canva.tool.yaml`

**Step 1: Write the Canva tool definition**

Create `_conectese/tools/registry/canva.tool.yaml` with this exact content:
...
Run: `cat _conectese/tools/registry/canva.tool.yaml`
Expected: The YAML above, with `id: canva` and `url: "https://mcp.canva.com/mcp"`

**Step 3: Commit**

```bash
git add _conectese/tools/registry/canva.tool.yaml
...
**Files:**
- Delete: `_conectese/tools/registry/figma.tool.yaml`

**Step 1: Delete the Figma tool definition**

```bash
rm _conectese/tools/registry/figma.tool.yaml
```
...
```bash
git add _conectese/tools/registry/figma.tool.yaml
...
**Files:**
- Create: `templates/_conectese/tools/registry/canva.tool.yaml`

**Step 1: Copy the Canva tool definition to templates**

```bash
cp _conectese/tools/registry/canva.tool.yaml templates/_conectese/tools/registry/canva.tool.yaml
```

**Step 2: Verify the copy is identical**

```bash
diff _conectese/tools/registry/canva.tool.yaml templates/_conectese/tools/registry/canva.tool.yaml
```
...
```bash
git add templates/_conectese/tools/registry/canva.tool.yaml
...
**Files:**
- Delete: `templates/_conectese/tools/registry/figma.tool.yaml`

**Step 1: Delete the template Figma tool definition**

```bash
rm templates/_conectese/tools/registry/figma.tool.yaml
```

**Step 2: Verify deletion**

Run: `ls templates/_conectese/tools/registry/`
Expected: `apify.tool.yaml` and `canva.tool.yaml` only

**Step 3: Commit**

```bash
git add templates/_conectese/tools/registry/figma.tool.yaml
...
**Files:**
- Modify: `_conectese/core/architect.agent.yaml:420`
- Modify: `templates/_conectese/core/architect.agent.yaml:420`

**Step 1: Update the live architect file**

In `_conectese/core/architect.agent.yaml`, find line 420:
...
In `templates/_conectese/core/architect.agent.yaml`, find the same line:
...
Run: `grep -n "canva\|figma" _conectese/core/architect.agent.yaml`
Expected: One line with `# - canva`, zero lines with `figma`

Run: `grep -n "canva\|figma" templates/_conectese/core/architect.agent.yaml`
Expected: Same result

**Step 4: Commit**

```bash
git add _conectese/core/architect.agent.yaml templates/_conectese/core/architect.agent.yaml
...
```bash
ls _conectese/tools/registry/
ls templates/_conectese/tools/registry/
```
